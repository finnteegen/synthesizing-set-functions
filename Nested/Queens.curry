...
unsafeP :: Int -> ST (STList Int) -> ST Bool
unsafeP l = applyST $ \zs -> case zs of
  Nil       -> Fail l
  Cons x xs -> Choice l (unsafe1P l x (Val 0) xs) (unsafeP l xs)

unsafe1P :: Int -> ST Int -> ST Int -> ST (STList Int) -> ST Bool
unsafe1P l fst d zs = unsafe1Case `applyST` zs
 where
  unsafe1Case Nil         = Fail l
  unsafe1Case (Cons x xs) = Choice l (unsafe1P l fst (lift1P (+1) d) xs)
                                     (unsafeTest fst x d)
...
unsafeS x = fromST_Bool_Bool (unsafeP 1 (toST_List_STList toST_Int_Int x))
