...
data ST a = Val a                -- a value (in head normal form)
          ...
          | Choice (ST a) (ST a) -- non-deterministic choice
...
applyST f (Choice s1 s2) = Choice (f `applyST` s1) (f `applyST` s2)
...
nfST Fail           = Fail
nfST Fail0          = Fail0
nfST (Choice s1 s2) = Choice (nfST s1) (nfST s2)
...
instance NF a => NF (STList a) where
  ...
  nf (Cons sx sxs) = case nfST sx of
    Choice s1 s2 -> Choice (nfST (Val (Cons s1 sxs))) (nfST (Val (Cons s2 sxs)))
    Fail         -> Fail
    Fail0        -> Fail0
    sy           -> case nfST sxs of
      Choice s1 s2 -> Choice (nfST (Val (Cons sy s1))) (nfST (Val (Cons sy s2)))
      Fail         -> Fail
      Fail0        -> Fail0
      sys          -> Val (Cons sy sys)
...
searchDFS st = maybe failed id (searchDFS' st)
 where
  ...
  searchDFS' Fail           = Just []
  searchDFS' Fail0          = Nothing
  searchDFS' (Choice s1 s2) = concVals (searchDFS' s1) (searchDFS' s2)
