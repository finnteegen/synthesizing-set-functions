...
unsafeP :: IDSupply -> ST (STList Int) -> ST Bool
unsafeP s = applyST $ \zs -> case zs of
  Nil       -> Fail
  Cons x xs -> Choice (uniqueID s) (unsafe1P (leftSupply s) x (Val 0) xs)
                                   (unsafeP (rightSupply s) xs)

unsafe1P :: IDSupply -> ST Int -> ST Int -> ST (STList Int) -> ST Bool
unsafe1P s fst d zs = unsafe1Case `applyST` zs
 where
  unsafe1Case Nil         = Fail
  unsafe1Case (Cons x xs) =
    Choice (uniqueID s) (unsafe1P (leftSupply s) fst (lift1P (+1) d) xs)
                        (unsafeTest fst x d)
   where unsafeTest fst' x' d' =
          ifP (eqP (absP (minusP fst' x')) (plusP d' (Val 1))) (Val True) Fail

-- Set function:
unsafeS :: [Int] -> Values Bool
unsafeS x =
  fromST_Bool_Bool (unsafeP initSupply (toST_List_STList toST_Int_Int x))
